<template>
  <div class="header">
    ...
    <div class="window-container">
      <div class="window-icon  codicon codicon-chrome-minimize" @click="events['win-minimize']()"></div>
      <div class="window-icon  codicon codicon-chrome-maximize"
        :class="['window-icon', 'codicon', maxWindow ? 'codicon-chrome-restore' : 'codicon-chrome-maximize']"
        @click="winMaxClick"></div>
      <div class="window-icon  codicon codicon-chrome-close" @click="events['win-close']()"></div>
    </div>
  </div>
  <ContextMenu
...
import events from '@commonUtils/window'
const btns = [
  {
    id: 1,
    label: '文件(F)'
...
const menuClick = (item: any, event: Event) => {
  console.log(event)
...
const contextMenuClick = (e: TabContextMenu) => {
  const { id, label } = e
