...
let defaultOption = {
  ...
  theme: 'vs-dark', // vs, hc-black, or vs-dark // 编辑器主题
...
const getFileUrl = (path: Array<string>): string => {
  return '/' + path.join('/')
}
const initEditor = (val?: string) => {
  ...
  const arr = Array.from(tabsStore.list.values())
  arr.forEach((item: Tab) => {
    let path = getFileUrl(item.path)
    if (!monaco.editor.getModels().find((model) => model.uri.path === path)) {
      monaco.editor.createModel(item.text, undefined, new monaco.Uri().with({ path: '/' + path }))
    }
  })
  openFile('/' + arr[arr.length - 1].name)
}
onMounted(() => {
  // @ts-ignore
  ...
  if (myRef.value) editor = monaco.editor.create(myRef.value, option)
  // console.log('3.-组件挂载到页面之后执行-------onMounted')
  initEditor()
})
watch(
  () => tabsStore.list.values(),
  () => {
    // console.log('父组件code改变')
    initEditor()
  }
)
defineExpose({ formatCode, getValue })
</script>
